#!/usr/bin/env python
"""Offline stljax pipeline for STL Animation Studio scenes.

The browser prototype (../index.html) and this pipeline share one scene JSON
schema, and both implement the same STL semantics: stljax computes the
robustness here; the browser hand-rolls identical soft/exact computations so
it can run live. Round-trip: author specs in the browser, "export", optimize
here with stljax + optax, then "import" the result back and the browser's
exact verifier should agree (certificate turns green with the solver off).
...
def build_terms(scene):
    """Compile each constraint into (exact_fn, soft_fn, weight, margin, name).
    ...
    for c in scene["constraints"]:
        ...
        if t == "reach":
            ...
            terms.append((ex, so, c["weight"], MARGIN, name))
            ...
            terms.append((ex, so, c["weight"], SMOOTH_MARGIN, name))
            ...
            terms.append((fn, fn, c["weight"], MARGIN, name))
...
# ---------------------------------------------------------------- optimize
def optimize(scene, steps=1500, lr=0.5, seed=0):
    ...
    def loss_fn(p):
        total = 0.0
        for _, soft, w, margin, _ in terms:
            rho = soft(p)
            total += w * jnp.sum(
    ...
    for k in range(steps):
        ...
        if k % 300 == 0 or k == steps - 1:
            print(f"  step {k:5d}  soft loss {float(l):10.2f}")

    scene = dict(scene)
    scene["optimized"] = np.round(
        np.asarray(pos).transpose(1, 0, 2), 2
    ).tolist()
    return scene


def report(scene, which="optimized"):
    src = scene.get(which) or [o["points"] for o in scene["objects"]]
    pos = jnp.array(src, dtype=jnp.float32).transpose(1, 0, 2)
    print(f"\nExact STL robustness ({which}):")
    worst = np.inf
    for exact, _, w, _, name in build_terms(scene):
        rho = float(jnp.min(exact(pos)))
        worst = min(worst, rho)
        print(f"  {'✓' if rho >= 0 else '✗'}  ρ = {rho:+8.2f}   {name} (w={w})")
    print(
